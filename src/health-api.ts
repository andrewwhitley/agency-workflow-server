...
import { FUNCTIONAL_RANGES, getMarkerReference, BODY_SYSTEMS, PROTOCOL_CATEGORIES, MEAL_TYPES, DIET_TAGS, BIOMARKER_CATEGORIES, getMarkersForBiomarkerCategory } from "./health-references.js";
import {
...
export function healthRouter(): Router {
  ...
  // ── Biomarker Summary by Category ───────────────────
  // Returns Function Health-style category breakdown with in-range/out-of-range/other counts

  router.get("/biomarkers/summary", async (req: Request, res: Response) => {
    try {
      const memberId = req.query.memberId as string;
      if (!memberId) { res.status(400).json({ error: "memberId required" }); return; }
      if (!(await verifyMemberOwnership(memberId, getUserEmail(req)))) { res.status(404).json({ error: "Not found" }); return; }

      // Get the most recent lab result with markers
      const { rows: labRows } = await query(
        `SELECT lr.id, lr.test_date, lr.lab_name,
           (SELECT json_agg(lm.* ORDER BY lm.category, lm.name) FROM lab_markers lm WHERE lm.lab_result_id = lr.id) as markers
         FROM lab_results lr WHERE lr.family_member_id = $1
         ORDER BY lr.test_date DESC LIMIT 1`,
        [memberId],
      );

      if (!labRows.length || !labRows[0].markers) {
        res.json({ categories: BIOMARKER_CATEGORIES, results: [], totalInRange: 0, totalOutOfRange: 0, totalOther: 0, totalMarkers: 0, testDate: null });
        return;
      }

      const markers = labRows[0].markers as Array<{
        name: string; value: number; unit: string; category: string;
        optimal_low: number | null; optimal_high: number | null;
        conventional_low: number | null; conventional_high: number | null;
      }>;

      let totalInRange = 0, totalOutOfRange = 0, totalOther = 0;

      const results = BIOMARKER_CATEGORIES.map((cat) => {
        const catMarkerNames = getMarkersForBiomarkerCategory(cat.id).map((m) => m.name);
        const matchedMarkers = markers.filter((m) => {
          // Match by category from reference data
          const ref = FUNCTIONAL_RANGES[m.name];
          return ref && cat.markerCategories.includes(ref.category);
        });

        let inRange = 0, outOfRange = 0, other = 0;
        const markerDetails: Array<{ name: string; value: number; unit: string; status: string; optimal_low: number | null; optimal_high: number | null }> = [];

        for (const m of matchedMarkers) {
          const optLow = m.optimal_low;
          const optHigh = m.optimal_high;
          let status = "other";
          if (optLow != null && optHigh != null) {
            if (m.value >= optLow && m.value <= optHigh) {
              status = "in-range";
              inRange++;
            } else {
              status = "out-of-range";
          ...
          markerDetails.push({ name: m.name, value: m.value, unit: m.unit, status, optimal_low: optLow, optimal_high: optHigh });
        }
        ...
        return {
          ...
          outOfRange,
          other,
          total: matchedMarkers.length,
          markers: markerDetails,
        };
      }).filter((r) => r.total > 0); // Only include categories that have tested markers
      ...
      res.json({
        ...
        totalOutOfRange,
        totalOther,
        totalMarkers: totalInRange + totalOutOfRange + totalOther,
        testDate: labRows[0].test_date,
        labName: labRows[0].lab_name,
      });
    } catch (err) { console.error("GET /biomarkers/summary:", err); res.status(500).json({ error: "Failed to load biomarker summary" }); }
  });

  // ── AI Food Recommendations ───────────────────────
  // Returns AI-generated foods to enjoy vs avoid based on lab data

  router.get("/foods/recommendations", async (req: Request, res: Response) => {
    try {
      const memberId = req.query.memberId as string;
      if (!memberId) { res.status(400).json({ error: "memberId required" }); return; }
      if (!(await verifyMemberOwnership(memberId, getUserEmail(req)))) { res.status(404).json({ error: "Not found" }); return; }

      // Get member info + recent labs
      const [memberResult, labResult] = await Promise.all([
        query("SELECT * FROM family_members WHERE id = $1", [memberId]),
        query(
          `SELECT lr.*, (SELECT json_agg(lm.* ORDER BY lm.category, lm.name) FROM lab_markers lm WHERE lm.lab_result_id = lr.id) as markers
           FROM lab_results lr WHERE lr.family_member_id = $1 ORDER BY lr.test_date DESC LIMIT 1`,
          [memberId],
        ),
      ]);

      const member = memberResult.rows[0];
      const markers = labResult.rows[0]?.markers || [];

      // Build context for food recommendations
      const outOfRangeMarkers = markers.filter((m: any) =>
        m.optimal_low != null && m.optimal_high != null &&
        (m.value < m.optimal_low || m.value > m.optimal_high)
      );

      res.json({
        member: { name: member.name, conditions: member.conditions, allergies: member.allergies },
        outOfRangeMarkers,
        allMarkers: markers,
      });
    } catch (err) { console.error("GET /foods/recommendations:", err); res.status(500).json({ error: "Failed to load" }); }
  });
