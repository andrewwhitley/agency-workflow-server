...
const SEO_SUBTAB_LABELS: Record<string, string> = {
  serp: "SERP Analysis", domain: "Domain Analytics", onpage: "On-Page Audit",
  content: "Content Analysis", business: "Business Listings",
};
...
function volClass(vol?: number) {
  if (vol == null) return "text-muted";
  if (vol > 100) return "text-success font-semibold";
  if (vol > 10) return "text-warning font-semibold";
  return "text-muted";
}

function diffClass(diff?: number) {
  if (diff == null) return "text-muted";
  if (diff < 30) return "text-success font-semibold";
  if (diff < 60) return "text-warning font-semibold";
  return "text-destructive font-semibold";
}

function StatCard({ value, label }: { value: string | number; label: string }) {
  return (
    <div className="bg-surface border border-border rounded-md p-4 flex-1 min-w-[140px]">
...
function TypeBadge({ type }: { type?: string }) {
  ...
  const colors: Record<string, string> = {
    ...
    area: "bg-warning/10 text-warning",
    blog: "bg-accent/10 text-accent",
    support: "bg-surface-2 text-dim",
  };
...
function CalendarTab({ slug }: { slug: string }) {
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      const res = await api<CalendarStrategy>(`/content-management/clients/${slug}/strategy`, { method: "POST" });
      setStrategy(res);
  ...
  const enrichWithSeo = async () => {
    ...
    try {
      ...
      for (const item of strategy.calendar) {
        if (item.primaryKeyword) keywordSet.add(item.primaryKeyword.toLowerCase());
        if (item.secondaryKeywords) {
          for (const sk of item.secondaryKeywords) keywordSet.add(sk.toLowerCase());
        }
      }
      const allKeywords = Array.from(keywordSet);
      if (!allKeywords.length) return;

      const result = await api<{ keywords: KeywordResult[] }>("/content-management/keywords/enrich", {
        method: "POST",
        body: JSON.stringify({ keywords: allKeywords }),
      });

      const metricsMap: Record<string, KeywordResult> = {};
      for (const m of result.keywords || []) {
        metricsMap[m.keyword.toLowerCase()] = m;
      }

      const now = new Date().toISOString();
      const updatedCalendar = strategy.calendar.map((item) => {
        const pk = (item.primaryKeyword || "").toLowerCase();
        const pkData = metricsMap[pk];
        const secondaryVols: Record<string, number> = {};
        if (item.secondaryKeywords) {
          for (const sk of item.secondaryKeywords) {
            const skData = metricsMap[sk.toLowerCase()];
            if (skData?.searchVolume) secondaryVols[sk] = skData.searchVolume;
          }
        }
        return {
          ...item,
          seoMetrics: {
            primaryKeywordVolume: pkData?.searchVolume,
            primaryKeywordDifficulty: pkData?.keywordDifficulty,
            primaryKeywordCpc: pkData?.cpc,
            primaryKeywordCompetition: pkData?.competitionLevel,
            secondaryKeywordVolumes: Object.keys(secondaryVols).length ? secondaryVols : undefined,
            enrichedAt: now,
          },
        };
      });
      setStrategy({ ...strategy, calendar: updatedCalendar });
    } catch (err) { console.error(err); alert("SEO enrichment failed"); }
    setEnriching(false);
  };
  ...
  const runKeywordSuggestions = async () => {
    ...
    setSuggesting(true);
    try {
      const data = await api<{ keywords: KeywordResult[] }>("/content-management/keywords/suggest", {
        method: "POST",
        body: JSON.stringify({ seed: seed.trim(), limit: 50 }),
      });
    ...
    <div>
      <div className="flex items-center gap-3 mb-4">
        <button onClick={generate} disabled={generating}
          className={cn("px-4 py-2 rounded-md text-sm font-medium", generating ? "bg-surface-2 text-dim" : "bg-accent text-white hover:bg-accent/90")}>
          {generating ? "Generating..." : "Generate 12-Month Calendar"}
        </button>
        {strategy?.calendar?.length ? (
          <button onClick={enrichWithSeo} disabled={enriching}
            className={cn("px-4 py-2 rounded-md text-sm font-medium", enriching ? "bg-surface-2 text-dim" : "bg-surface-2 text-foreground hover:bg-surface-3")}>
      ...
      {strategy?.summary && (
        <div className="flex gap-4 flex-wrap mb-6">
          <StatCard value={strategy.summary.totalPages || 0} label="Total Pages" />
          <StatCard value={strategy.summary.blogPosts || 0} label="Blog Posts" />
          <StatCard value={strategy.summary.caseStudies || 0} label="Case Studies" />
        </div>
      )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-surface-2">
                      ...
                      {hasMetrics && <>
                        <th className="text-left px-3 py-2 font-medium text-dim">Vol</th>
                        <th className="text-left px-3 py-2 font-medium text-dim">Diff</th>
                  ...
                  <tbody>
                    {items.map((item, i) => (
                      <tr key={i} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                        ...
                        {hasMetrics && <>
                          <td className={cn("px-3 py-2 text-xs", volClass(item.seoMetrics?.primaryKeywordVolume))}>
                            {item.seoMetrics?.primaryKeywordVolume != null ? item.seoMetrics.primaryKeywordVolume.toLocaleString() : "—"}
                          </td>
                          <td className={cn("px-3 py-2 text-xs", diffClass(item.seoMetrics?.primaryKeywordDifficulty))}>
                            {item.seoMetrics?.primaryKeywordDifficulty ?? "—"}
                          </td>
                          <td className="px-3 py-2 text-xs text-muted">
                            {item.seoMetrics?.primaryKeywordCpc != null ? `$${item.seoMetrics.primaryKeywordCpc.toFixed(2)}` : "—"}
                          </td>
                        </>}
      ...
      <div className="mt-8 pt-6 border-t border-border">
        ...
        <div className="flex gap-2 mb-4">
          <input type="text" value={seed} onChange={(e) => setSeed(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && runKeywordSuggestions()}
            placeholder="Enter a seed keyword (e.g. chiropractic care)"
          ...
          <button onClick={runKeywordSuggestions} disabled={suggesting}
            className={cn("px-4 py-2 rounded-md text-sm font-medium", suggesting ? "bg-surface-2 text-dim" : "bg-accent text-white hover:bg-accent/90")}>
            {suggesting ? "Searching..." : "Get Suggestions"}
          </button>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-2">
                  ...
                  <th className="text-left px-3 py-2 font-medium text-dim">CPC</th>
                  <th className="text-left px-3 py-2 font-medium text-dim">Competition</th>
                </tr>
              </thead>
              <tbody>
                {suggestions.map((kw, i) => (
                  <tr key={i} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                    ...
                    <td className={cn("px-3 py-2 text-xs", volClass(kw.searchVolume))}>{(kw.searchVolume || 0).toLocaleString()}</td>
                    <td className={cn("px-3 py-2 text-xs", diffClass(kw.keywordDifficulty))}>{kw.keywordDifficulty ?? "—"}</td>
                    <td className="px-3 py-2 text-xs text-muted">${(kw.cpc || 0).toFixed(2)}</td>
                    <td className="px-3 py-2 text-xs text-muted">{kw.competitionLevel || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  const generate = async () => {
    ...
    try {
      await api(`/content-management/clients/${slug}/generate`, {
        method: "POST",
        body: JSON.stringify({ pages: Array.from(selected), qaEnabled }),
      });
...
function SerpPanel() {
  ...
  const search = async () => {
    ...
    setLoading(true);
    try {
      const res = await api<typeof data>("/seo/serp", { method: "POST", body: JSON.stringify({ keyword: keyword.trim() }) });
      setData(res);
    } catch (err) { console.error(err); }
    setLoading(false);
  };
  ...
  const searchRelated = (term: string) => {
    ...
    setTimeout(() => {
      setLoading(true);
      api<typeof data>("/seo/serp", { method: "POST", body: JSON.stringify({ keyword: term }) })
        ...
        <div className="space-y-4">
          ...
          {data.peopleAlsoAsk && data.peopleAlsoAsk.length > 0 && (
            <div className="bg-surface-2 rounded-md p-3">
              <div className="text-sm font-semibold text-foreground mb-2">People Also Ask</div>
          ...
          <div className="bg-surface border border-border rounded-md overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-2">
                  <th className="text-center px-3 py-2 font-medium text-dim w-12">#</th>
                  <th className="text-left px-3 py-2 font-medium text-dim">Title</th>
                  <th className="text-left px-3 py-2 font-medium text-dim">Type</th>
                  <th className="text-left px-3 py-2 font-medium text-dim">URL</th>
                </tr>
              </thead>
              <tbody>
                {(data.items || []).map((item, i) => (
                  <tr key={i} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                    <td className="px-3 py-2 text-center font-semibold text-dim">{item.position}</td>
                    <td className="px-3 py-2 font-medium text-foreground">{item.title}</td>
                    <td className="px-3 py-2"><TypeBadge type={item.type === "organic" ? "core" : item.type === "local_pack" ? "area" : "support"} /></td>
                    <td className="px-3 py-2 text-xs max-w-[300px] truncate">
                      {item.url && <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">{item.domain || item.url}</a>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {data.relatedSearches && data.relatedSearches.length > 0 && (
            <div className="bg-surface-2 rounded-md p-3">
              <div className="text-sm font-semibold text-foreground mb-2">Related Searches</div>
              <div className="flex flex-wrap gap-2">
                {data.relatedSearches.map((rs, i) => (
                  <button key={i} onClick={() => searchRelated(rs)}
                    className="px-2 py-1 rounded text-xs bg-surface border border-border text-foreground hover:border-accent transition-colors">
                    {rs}
                  </button>
                ))}
              </div>
  ...
  const analyze = async (d?: string) => {
    ...
    try {
      const [ov, kw, comp] = await Promise.all([
        api<{ organicTraffic?: number; organicKeywords?: number; rank?: number }>("/seo/domain/overview", { method: "POST", body: JSON.stringify({ domain: target }) }),
      ...
      {overview && (
        <div className="space-y-6">
          <div className="flex gap-4 flex-wrap">
            <StatCard value={(overview.organicTraffic || 0).toLocaleString()} label="Est. Monthly Traffic" />
            <StatCard value={(overview.organicKeywords || 0).toLocaleString()} label="Ranked Keywords" />
            <StatCard value={(overview.rank || 0).toLocaleString()} label="Domain Rank" />
          </div>

          {keywords.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-2">Top Ranked Keywords</h3>
              <div className="bg-surface border border-border rounded-md overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-surface-2">
                      <th className="text-left px-3 py-2 font-medium text-dim">Keyword</th>
                      <th className="text-center px-3 py-2 font-medium text-dim">Pos</th>
                      <th className="text-left px-3 py-2 font-medium text-dim">Vol</th>
                      <th className="text-left px-3 py-2 font-medium text-dim">CPC</th>
                      <th className="text-left px-3 py-2 font-medium text-dim">URL</th>
                    </tr>
                  ...
                  <tbody>
                    {keywords.map((kw, i) => (
                      <tr key={i} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                        <td className="px-3 py-2 font-medium text-foreground">{kw.keyword}</td>
                        <td className="px-3 py-2 text-center font-semibold">{kw.position}</td>
                        <td className={cn("px-3 py-2 text-xs", volClass(kw.searchVolume))}>{(kw.searchVolume || 0).toLocaleString()}</td>
                        <td className="px-3 py-2 text-xs text-muted">${(kw.cpc || 0).toFixed(2)}</td>
                        <td className="px-3 py-2 text-xs max-w-[200px] truncate">
                          {kw.url && <a href={kw.url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">{kw.url.replace(/^https?:\/\/[^/]+/, "")}</a>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {competitors.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-2">Competitors</h3>
              <div className="bg-surface border border-border rounded-md overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-surface-2">
                      <th className="text-left px-3 py-2 font-medium text-dim">Domain</th>
                      <th className="text-center px-3 py-2 font-medium text-dim">Avg Position</th>
                      <th className="text-left px-3 py-2 font-medium text-dim">Keyword Overlap</th>
                      <th className="text-left px-3 py-2 font-medium text-dim">Est. Traffic</th>
                    </tr>
                  </thead>
                  <tbody>
                    {competitors.map((c, i) => (
                      <tr key={i} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                        <td className="px-3 py-2">
                          <button onClick={() => analyze(c.domain)} className="text-accent hover:underline font-medium">{c.domain}</button>
                        </td>
                        <td className="px-3 py-2 text-center">{(c.avgPosition || 0).toFixed(1)}</td>
                        <td className="px-3 py-2">{(c.keywordIntersections || 0).toLocaleString()}</td>
                        <td className="px-3 py-2">{c.organicTraffic ? c.organicTraffic.toLocaleString() : "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function OnPagePanel() {
  ...
  const [data, setData] = useState<{
    onpageScore?: number; statusCode?: number; loadTime?: number; size?: number;
    title?: string; description?: string; h1?: string[];
    brokenLinks?: number; brokenResources?: number; duplicateTitle?: boolean; duplicateDescription?: boolean;
  ...
  const audit = async () => {
    ...
    setLoading(true);
    try {
      const res = await api<typeof data>("/seo/onpage", { method: "POST", body: JSON.stringify({ url: url.trim() }) });
      setData(res);
    } catch (err) { console.error(err); }
    setLoading(false);
  };
    ...
    <div>
      ...
      {data && (
        <div className="space-y-6">
          <div className="flex gap-4 flex-wrap">
            <div className="bg-surface border border-border rounded-md p-4 flex-1 min-w-[140px]">
              <div className={cn("text-4xl font-bold", scoreColor(data.onpageScore))}>{(data.onpageScore || 0).toFixed(0)}</div>
              <div className="text-xs text-muted">SEO Score</div>
            </div>
            <StatCard value={data.statusCode || 0} label="Status Code" />
            <StatCard value={`${((data.loadTime || 0) * 1000).toFixed(0)}ms`} label="Load Time" />
            <StatCard value={`${((data.size || 0) / 1024).toFixed(0)}KB`} label="Page Size" />
          </div>

          <div>
            <h3 className="text-sm font-semibold text-foreground mb-2">Meta Tags</h3>
            <div className="bg-surface-2 rounded-md p-4 text-sm space-y-2">
              <div><span className="font-semibold">Title:</span> {data.title || "Missing"}</div>
              <div><span className="font-semibold">Description:</span> {data.description || "Missing"}</div>
              <div><span className="font-semibold">H1:</span> {data.h1?.length ? data.h1.join(", ") : "Missing"}</div>
            </div>
          </div>

          {(data.brokenLinks || data.brokenResources || data.duplicateTitle || data.duplicateDescription) && (
            <div>
              <h3 className="text-sm font-semibold text-destructive mb-2">Issues Found</h3>
              <div className="flex flex-wrap gap-2">
                {!!data.brokenLinks && <span className="text-xs px-2 py-1 rounded bg-destructive/10 text-destructive">{data.brokenLinks} broken links</span>}
                {!!data.brokenResources && <span className="text-xs px-2 py-1 rounded bg-destructive/10 text-destructive">{data.brokenResources} broken resources</span>}
                {data.duplicateTitle && <span className="text-xs px-2 py-1 rounded bg-warning/10 text-warning">Duplicate title</span>}
                {data.duplicateDescription && <span className="text-xs px-2 py-1 rounded bg-warning/10 text-warning">Duplicate description</span>}
              </div>
            </div>
          )}
          ...
          {data.checks && (
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-2">Technical Checks</h3>
              <div className="flex flex-wrap gap-2">
                {Object.entries(data.checks).map(([key, val]) => {
                  ...
                  return (
                    <span key={key} className={cn("text-xs px-2 py-1 rounded",
...
function ContentAnalysisPanel() {
  ...
  const search = async () => {
    ...
    setLoading(true);
    try {
      const data = await api<{ results?: ContentResult[] }>("/seo/content", { method: "POST", body: JSON.stringify({ keyword: keyword.trim(), limit: 20 }) });
  ...
  return (
    <div>
      <div className="flex gap-2 mb-4">
        <input type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && search()}
          placeholder="Search for content about..."
          className="flex-1 px-3 py-2 rounded-md border border-border bg-surface text-foreground text-sm focus:outline-none focus:border-accent" />
        <button onClick={search} disabled={loading}
          className={cn("px-4 py-2 rounded-md text-sm font-medium", loading ? "bg-surface-2 text-dim" : "bg-accent text-white hover:bg-accent/90")}>
          {loading ? "Searching..." : "Search"}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-surface-2">
                <th className="text-left px-3 py-2 font-medium text-dim">Title</th>
                <th className="text-left px-3 py-2 font-medium text-dim">URL</th>
                <th className="text-left px-3 py-2 font-medium text-dim">Published</th>
                <th className="text-left px-3 py-2 font-medium text-dim">Length</th>
              </tr>
            </thead>
            <tbody>
              {results.map((item, i) => {
                const date = item.datePublished ? new Date(item.datePublished).toLocaleDateString() : "—";
                const len = item.contentLength ? (item.contentLength > 1000 ? `${(item.contentLength / 1000).toFixed(1)}K` : item.contentLength) + " chars" : "—";
                return (
                  <tr key={i} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                    <td className="px-3 py-2 font-medium text-foreground max-w-[300px] truncate">{item.title}</td>
                    <td className="px-3 py-2 text-xs max-w-[250px] truncate">
                      {item.url && <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">{item.url.replace(/^https?:\/\//, "").slice(0, 60)}</a>}
                    </td>
                    <td className="px-3 py-2 text-xs text-muted">{date}</td>
                    <td className="px-3 py-2 text-xs text-muted">{len}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {!loading && results.length === 0 && keyword && <div className="text-sm text-muted">No content found.</div>}
    </div>
  );
}
...
function BusinessPanel() {
  ...
  const search = async () => {
    ...
    setLoading(true);
    try {
      const data = await api<{ listings?: BusinessListing[] }>("/seo/business", { method: "POST", body: JSON.stringify({ keyword: keyword.trim(), limit: 20 }) });
  ...
  return (
    <div>
      <div className="flex gap-2 mb-4">
        <input type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && search()}
          placeholder="Search businesses (e.g. chiropractor phoenix az)"
          className="flex-1 px-3 py-2 rounded-md border border-border bg-surface text-foreground text-sm focus:outline-none focus:border-accent" />
        <button onClick={search} disabled={loading}
          className={cn("px-4 py-2 rounded-md text-sm font-medium", loading ? "bg-surface-2 text-dim" : "bg-accent text-white hover:bg-accent/90")}>
          {loading ? "Searching..." : "Search"}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {listings.map((biz, i) => (
            <div key={i} className="bg-surface border border-border rounded-md p-4">
              <div className="font-semibold text-sm text-foreground mb-1">{biz.title}</div>
              {biz.rating != null && (
                <div className="flex items-center gap-1 mb-2">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <span key={s} className={s <= Math.round(biz.rating!) ? "text-warning" : "text-dim"}>&#9733;</span>
                  ))}
                  <span className="text-xs text-muted ml-1">{biz.rating.toFixed(1)} ({biz.reviewCount || 0})</span>
                </div>
              )}
              ...
              {biz.category && <span className="text-xs px-2 py-0.5 rounded bg-accent/10 text-accent">{biz.category}</span>}
              {biz.url && <div className="mt-2"><a href={biz.url} target="_blank" rel="noopener noreferrer" className="text-xs text-accent hover:underline">Website</a></div>}
            </div>
          ))}
        </div>
      )}
      {!loading && listings.length === 0 && keyword && <div className="text-sm text-muted">No listings found.</div>}
    </div>
  );
}
  ...
  return (
    <div>
      <div className="grid grid-cols-3 gap-4">
        <StatCard value={queue.pending} label="Pending" />
        <StatCard value={queue.running} label="Running" />
        <StatCard value={queue.completed} label="Completed" />
      </div>
    </div>
  );
}
